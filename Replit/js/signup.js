...
async function verifyPassword(user, passwordField, confirmPasswordField) {
  ...
  if (password === confirmPassword) {
    try {
      await api.signup(user.name, user.mail, password);
      return true;
    } catch (error) {
      displayErrorMessage(error.message || "Signup failed", passwordField);
      return false;
    }
  } else {
...
async function login() {
  ...
  try {
    ...
    displayErrorMessageLogin("E-Mail or password are incorrect");
  }
}
